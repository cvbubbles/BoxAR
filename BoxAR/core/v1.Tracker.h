...
public:
	static void _sample(Rect roiRect, const Mat1f& depth, CVRProjector& prj, std::vector<CPoint>& c3d, int nSamples)
	{
		...
		auto get3D = [&depthMask, &depth, dstride, &prj, roiRect](int x, int y, Point3f& P, float& _z) {
			...
			{
				CV_Assert(false);
				//enum { hwsz = 0, wsz = 2 * hwsz + 1 };

				//const uchar* mp = depthMask.ptr(y - hwsz, x - hwsz);
				//const float* dp = depth.ptr<float>(y - hwsz, x - hwsz);

				//float dn = 0;

				//for (int dy = 0; dy < wsz; ++dy, mp += depthMask.step, dp += dstride)
				//{
...
public:
	virtual void loadModel(const std::string& modelFile, const std::string& argstr)
	{
		...
		_useInnerSeg = args.getd<bool>("useInnserSeg", false);
	}
	...
	virtual void startUpdate(const Mat& img, const Mat1f& segMask, Pose gtPose = Pose())
	{
		//g_gtPose = _scalePose(gtPose);
